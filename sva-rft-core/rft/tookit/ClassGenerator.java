...
public class ClassGenerator 
{
	 ...
	 * so your code will look something like this:<br>
	 * <code>
	 * 		<dd>Vector v = new Vector();<br>
	 * 		<dd>v.add(new LoginPage());<br>
	 * 		<dd>v.add(new SearchPage());<br>
	 * 		<dd>new ClassGenerator().updateScripts(v);<br>
	 * </code>
	 * 
	 * @param testScripts - a vector including the test scripts for which you wish to create methods
	 */
	public void updateScripts(Vector<RationalTestScript> testScripts) {
		for(int i = 0; i < testScripts.size(); i++) {
			RationalTestScript ts = (RationalTestScript)testScripts.elementAt(i);
	...
	public String generateClass(RationalTestScript ts)
	{
		IScriptDefinition sd = ts.getScriptDefinition();
		Enumeration<String> e = sd.getTestObjectNames();
		//Vector<RationalTestScript> v = new Vector<>();
	...
	{
		return "import rft.widgets.*;\n"
			+ "import rft.widgets.ancestors.*;\n"
			//+ "import rft.widgets.swt.*;\n"
			+ "\n"
	...
	public void updateFile(RationalTestScript ts, String newFileContents)
	{
	...
	public String generateMethod(String objName, String objClassName, String className, String role, int numTabs)		
	{
		...
		//ANY is a constant in RationalTestScript, and NO_STATE is a constant in the RationalTestScriptConstants interface that just mirrors ScriptCommandFlags.NO_FLAGS.
		methodText += ")" + " {\n"
	...
	protected String getWClassName(String objClassName, String className, String role)
	{
		...
		else if(role.equals("Text") || role.equals("Label"))
		{
		    // Added by Paul Chiang to handle Terminal Application classes
		    if(className.equals("com.ibm.eNetwork.ECL.ECLField"))
		        return "TATextField";
		 	return "WTextField";
		}  
